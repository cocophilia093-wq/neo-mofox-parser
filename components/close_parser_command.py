...
logger = get_logger("parser.close_parser_command")


class CloseParserCommand(BaseCommand):
    """关闭当前会话的链接解析。"""

    command_name: str = "关闭解析"
    command_description: str = "将当前会话加入 parser 黑名单（仅管理员）"
    ...
    async def handle(self) -> tuple[bool, str]:
        ...
        cfg.add_blacklist(self.stream_id)
        await send_text("当前会话的解析已关闭", stream_id=self.stream_id)
        logger.info(f"[parser] 已加入黑名单: {self.stream_id}")
        return True, "ok"
